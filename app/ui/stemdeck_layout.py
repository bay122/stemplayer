...
from app.ui.deck_volume_slider import DeckVolumeSlider
from app.ui.deck_pan_slider import DeckPanSlider
...
class StemDeckLayout(QWidget):
    ...
    def _build_player_section(self):
        """El reproductor del deck: layout de 3 columnas + fila de waveform.

        Layout inspired by OTROS/ejemplo_distribucion_reproductor.html:

        - ZONE-LEFT: master volume (DeckVolumeSlider) + count-in combo.
        - ZONE-CENTER: transport buttons + time labels stacked.
        - ZONE-RIGHT: undo/redo/reset, heart (favorite), metronome checkbox,
          and (when enabled) metro volume + pan sliders.
        - WAVEFORM-ROW: current time, global waveform, total time.
        """
        main = self.main
        section = QFrame()
        ...
        # ---- Row 1: 3-column controls ----
        controls_row = QHBoxLayout()
        controls_row.setSpacing(20)
        outer.addLayout(controls_row)

        # ---- ZONE-LEFT: master + metrónomo ----
        left_col = QVBoxLayout()
        left_col.setSpacing(10)
        left_col.setContentsMargins(20, 15, 20, 15)
        ...
        self.deck_master_slider = DeckVolumeSlider(
            parent=section, icons_dir=self.icons_dir
        )
		...
		# Row: metronome checkbox
        metro_check_row = QHBoxLayout()
        metro_check_row.setSpacing(6)
        metro_check_row.setAlignment(Qt.AlignRight)
        self.deck_click_check = QCheckBox("Metrónomo")
        self.deck_click_check.setToolTip(
            "Metrónomo persistente durante la canción"
        )
        self.deck_click_check.setChecked(self.main.state.click_during_playback)
        self.deck_click_check.stateChanged.connect(self._on_metronome_toggled)
        metro_check_row.addWidget(self.deck_click_check)
        left_col.addLayout(metro_check_row)

        # Row: metro volume + pan (shown only when metronome enabled)
        metro_sliders_row = QHBoxLayout()
        metro_sliders_row.setSpacing(8)
        metro_sliders_row.setAlignment(Qt.AlignRight)

        metro_vol_lbl = QLabel("Vol")
        metro_vol_lbl.setStyleSheet(
            f"color: {theme.TEXT_SECONDARY}; font-size: 11px; "
            f"background: transparent;"
        )
        metro_sliders_row.addWidget(metro_vol_lbl)
        self.deck_metro_vol_slider = DeckVolumeSlider(
            parent=section, icons_dir=self.icons_dir
        )
        self.deck_metro_vol_slider.setValue(self.main.state.metronome_volume)
        self.deck_metro_vol_slider.valueChanged.connect(
            self.main._on_metronome_volume_changed
        )
        self.deck_metro_vol_slider.sliderReleased.connect(
            self.main._on_metronome_volume_released
        )
        self.deck_metro_vol_slider.setVisible(self.main.state.click_during_playback)
        self.deck_metro_vol_slider.setFixedWidth(120)
        metro_sliders_row.addWidget(self.deck_metro_vol_slider)

        metro_pan_lbl = QLabel("Pan")
        metro_pan_lbl.setStyleSheet(
            f"color: {theme.TEXT_SECONDARY}; font-size: 11px; "
            f"background: transparent;"
        )
        metro_sliders_row.addWidget(metro_pan_lbl)
        self.deck_metro_pan_slider = DeckPanSlider(
            parent=section, icons_dir=self.icons_dir
        )
        self.deck_metro_pan_slider.setValue(self.main.state.metronome_pan)
        self.deck_metro_pan_slider.valueChanged.connect(
            self.main._on_metronome_pan_changed
        )
        self.deck_metro_pan_slider.sliderReleased.connect(
            self.main._on_metronome_pan_released
        )
        self.deck_metro_pan_slider.setVisible(self.main.state.click_during_playback)
        self.deck_metro_pan_slider.setFixedWidth(120)
        metro_sliders_row.addWidget(self.deck_metro_pan_slider)

        left_col.addLayout(metro_sliders_row)

        left_container = QWidget()
        left_container.setLayout(left_col)
        left_container.setFixedWidth(400)
        controls_row.addWidget(left_container, 0)

        # ---- ZONE-CENTER: transport + time ----
        ...
        self.deck_prev_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-prev.svg"))
        )
        ...
        self.deck_play_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-play.svg"))
        )
        self.deck_play_btn.setFixedSize(42, 42)
        ...
        self.deck_stop_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-stop.svg"))
        )
        ...
        self.deck_next_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-next.svg"))
        )
        ...
        self.deck_auto_play_btn.setIcon(
            svg_icon(
                os.path.join(self.icons_dir, "fad-preset-ab.svg"),
                theme.SVG_ICON_ACTIVE,
            )
        )
        self.deck_auto_play_btn.setFixedSize(34, 34)
        self.deck_auto_play_btn.setCheckable(True)
        self.deck_auto_play_btn.setToolTip(
            "Auto-avanzar y reproducir en setlist"
        )
        self.deck_auto_play_btn.toggled.connect(self.main._on_auto_play_toggled)
        trans_row.addWidget(self.deck_auto_play_btn)

        center_col.addLayout(trans_row)
        ...
        self.deck_current_time.setFixedWidth(50)
        self.deck_current_time.setAlignment(Qt.AlignCenter)
        ...
        self.deck_total_time.setFixedWidth(50)
        self.deck_total_time.setAlignment(Qt.AlignCenter)
        time_row.addWidget(self.deck_total_time)
        center_col.addLayout(time_row)

        center_container = QWidget()
        center_container.setLayout(center_col)
        controls_row.addWidget(center_container, 1)

        # ---- ZONE-RIGHT: undo/redo/reset + heart + metronome ----
        right_col = QVBoxLayout()
        right_col.setSpacing(8)
        right_col.setContentsMargins(20, 15, 20, 25)
        right_col.setAlignment(Qt.AlignCenter)

        # Row: undo, redo, reset, heart
        actions_row = QHBoxLayout()
        actions_row.setSpacing(6)
        actions_row.setAlignment(Qt.AlignRight)

        self.deck_undo_btn = QPushButton()
        self.deck_undo_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-undo.svg"))
        )
        ...
        actions_row.addWidget(self.deck_undo_btn)

        self.deck_redo_btn = QPushButton()
        self.deck_redo_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-redo.svg"))
        )
        ...
        actions_row.addWidget(self.deck_redo_btn)

        self.deck_reset_btn = QPushButton()
        self.deck_reset_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-history.svg"))
        )
        self.deck_reset_btn.setFixedSize(32, 32)
        self.deck_reset_btn.setToolTip(
            "Restablecer todos los cambios de pitch y volumen"
        )
        self.deck_reset_btn.clicked.connect(self.main._reset_all)
        actions_row.addWidget(self.deck_reset_btn)

        self.deck_heart_btn = QPushButton()
        self.deck_heart_btn.setIcon(
            svg_icon(os.path.join(self.icons_dir, "fad-heart-1.svg"))
        )
        self.deck_heart_btn.setFixedSize(30, 30)
        self.deck_heart_btn.setCheckable(True)
        self.deck_heart_btn.setToolTip("Marcar como favorita")
        self.deck_heart_btn.clicked.connect(self._on_favorite_toggled)
        self._refresh_favorite_state()
        actions_row.addWidget(self.deck_heart_btn)
        ...
        cin_lbl.setStyleSheet(
            f"color: {theme.TEXT_SECONDARY}; font-size: 11px; "
            f"background: transparent; min-width: 48px;"
        )
        cin_row.addWidget(cin_lbl)
        self.deck_count_in_combo = QComboBox()
        self.deck_count_in_combo.addItems(
            ["Sin count-in", "1 compás", "2 compases"]
        )
        self.deck_count_in_combo.setCurrentIndex(self.main.state.count_in_bars)
        self.deck_count_in_combo.currentIndexChanged.connect(
            self.main._on_count_in_changed
        )
        self.deck_count_in_combo.setMinimumHeight(26)
        cin_row.addWidget(self.deck_count_in_combo, 1)
        right_col.addLayout(cin_row)

        right_container = QWidget()
        right_container.setLayout(right_col)
        right_container.setFixedWidth(300)
        controls_row.addWidget(right_container, 0)

        # ---- Row 2: waveform + time labels ----
        from PySide6.QtWidgets import QSlider
        waveform_row = QHBoxLayout()
        waveform_row.setSpacing(6)
        waveform_row.setContentsMargins(0, 0, 0, 0)

        # Hidden slider kept for signal routing (the visual is the waveform)
        ...
        self._global_waveform.setMinimumHeight(48)
        self._global_waveform.setMaximumHeight(64)
        self._global_waveform.seek_requested.connect(self.main._on_waveform_seek)

        waveform_row.addWidget(self._global_waveform, 1)
    ...
    def _on_metronome_toggled(self, state):
        """Update internal state, show/hide metro sliders, and forward to main."""
        checked = bool(state)
        self.main.state.click_during_playback = checked
        self.deck_metro_vol_slider.setVisible(checked)
        self.deck_metro_pan_slider.setVisible(checked)
        # Forward to the main handler (which pushes to undo/redo, updates classic view)
        self.main._on_click_during_changed(state)

    def _on_favorite_toggled(self):
        """Toggle the current song's favorite state, update the heart button,
        and refresh the favorites section in the library panel so the new
        entry appears (or disappears) immediately.
        """
        song_name = self.main.state.current_song_name
        if not song_name:
            self.deck_heart_btn.setChecked(False)
            return
        config_mgr = self.main.config_mgr
        if self.deck_heart_btn.isChecked():
            config_mgr.add_favorite(song_name)
        else:
            config_mgr.remove_favorite(song_name)
        # Update the heart icon to match the new state
        self._refresh_favorite_state()
        # Refresh the favorites section in the library panel so the song
        # is added/removed from the visible list immediately (matches
        # the behavior of the context menu in library_panel.py).
        library_widget = getattr(self.main, 'library_widget', None)
        if library_widget is not None and hasattr(library_widget, '_refresh_favorites'):
            library_widget._refresh_favorites()

    def _refresh_favorite_state(self):
        """Read the favorite state from config and reflect it on the heart button."""
        song_name = self.main.state.current_song_name
        is_fav = False
        if song_name and self.main.config_mgr is not None:
            is_fav = self.main.config_mgr.is_favorite(song_name)
        self.deck_heart_btn.blockSignals(True)
        self.deck_heart_btn.setChecked(is_fav)
        if is_fav:
            self.deck_heart_btn.setIcon(
                svg_icon(os.path.join(self.icons_dir, "fad-heart-2.svg"))
            )
        else:
            self.deck_heart_btn.setIcon(
                svg_icon(os.path.join(self.icons_dir, "fad-heart-1.svg"))
            )
        self.deck_heart_btn.setStyleSheet(
            f"QPushButton {{"
            ...
            f"  color: #ff4d4d;"
            f"  border: 1px solid #ff4d4d;"
            f"  background-color: rgba(255, 77, 77, 0.10);"
            f"}}"
            f"QPushButton:hover {{"
            f"  color: #ff4d4d;"
            f"  border: 1px solid #ff4d4d;"
        ...
        self.deck_heart_btn.blockSignals(False)
