...
class SetlistPanel(QWidget):
	...
	def _build_ui(self):
		...
		setlist_layout = QVBoxLayout(setlist_content)
		setlist_layout.setContentsMargins(0, 0, 0, 0)
		setlist_layout.setSpacing(4)
		...
		self._section.toggled.connect(self._update_stretches)
		self._update_stretches()

	def _update_stretches(self):
		expanded = not self._section._collapsed
		if expanded:
	...
	def _create_setlist(self):
		...
		if ok and selected:
			self.config_mgr.add_setlist( name, [selected])
	...
	def _add_song_to_setlist_dialog(self):
		...
		from app.ui.add_song_dialog import AddSongToSetlistDialog
	...
	def add_song_to_current(self, song_name: str) -> bool:
		...
		self.config_mgr.update_setlist( self.current_setlist_index, sl["name"], sl["song_ids"])
		self.refresh_ui()
		return True
	...
	def _delete_setlist(self):
		...
		if self.current_setlist_index < len(setlists):
			...
			if reply == QMessageBox.Yes:
				self.config_mgr.remove_setlist( self.current_setlist_index)
				self.current_setlist_index = -1
				self.refresh_ui()
	...
	def _save_changes(self):
		...
		self.config_mgr.update_setlist( self.current_setlist_index, sl["name"], new_ids)
		self.save_btn.setEnabled(False)
